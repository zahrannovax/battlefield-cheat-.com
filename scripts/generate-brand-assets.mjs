...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'battlefield-6-cheats-logo.png'), logoPng);
	console.log('Wrote public/images/battlefield-6-cheats-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'battlefield-6-cheats-logo.webp'), logoWebp);
	console.log('Wrote public/images/battlefield-6-cheats-logo.webp');
}
...
async function generateWebManifest() {
	const manifest = {
		...
		short_name: 'Battlefield 6 Cheats',
		description: 'Undetected battlefield 6 cheats — ESP, aimbot, radar and for PC',
		start_url: '/',
		display: 'standalone',
